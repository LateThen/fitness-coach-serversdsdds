...
export const refresh = (req: Request, res: Response) => {
  if(!req.cookies.jwt) return res.status(406).json({ msg: "Unauthorised"})
  const refreshToken = req.cookies.jwt;
  jwt.verify(
    refreshToken,
    process.env.REFRESH_SECRET as string,
    (err: any, decoded: any) => {
    if (err) return res.status(406).json({ msg: "Unauthorised"});
    const accessToken = jwt.sign ({
      email: decoded.email
    },
    process.env.ACCESS_SECRET as string,
...
export const verify = (req: Request, res: Response, next: NextFunction) => {
 ...
 if (!token) return res.status(406).send({msg: "Unauthorised"})
 jwt.verify(
  token,
  process.env.ACCESS_SECRET as string,
  (err: any, user: any) => {
    if (err) return res.status(406).send({msg: "Unauthorised"})
